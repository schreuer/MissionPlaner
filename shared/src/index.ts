...
export interface MissionPreset {
  ...
  /** Optional higher-tier targets that surpass the minimal setup. */
  optimalShipRequirements?: ShipRequirement[];
  optimalRoleRequirements?: RoleRequirement[];
}
...
export interface GoalProgress {
  ...
  allMet: boolean;
  /** Whether the preset defines an optimal (higher-tier) target. */
  hasOptimal: boolean;
  totalOptimalShipsRequired: number;
  totalOptimalShipsMet: number;
  optimalShipsByRole: Array<{
    role: ShipRole;
    required: number;
    current: number;
    met: boolean;
  }>;
  totalOptimalRolesRequired: number;
  totalOptimalRolesMet: number;
  optimalRoleProgress: Array<{
    role: Role;
    required: number;
    current: number;
    met: boolean;
  }>;
  optimalAllMet: boolean;
}
...
export function computeGoalProgress(
  ...
  const optimalAllMet =
    optimalShipsByRole.every((s) => s.met) && optimalRoleProgress.every((r) => r.met);
